...
class Index extends PageRender {
		...
		try {
			...
			if (notificationLines.length > 0 && notificationLines[0] != '') {
				for (
					...
					notificationIndex < notificationLines.length;
					notificationIndex++
				) {
					...
					notificationHtml +=
						'<div class="carousel-cell"><div class="update-panel"><div class="update-text"><span class="update-date"> update ' +
						date +
						'</span><br> <span class="update-message">' +
						line +
						'</span></div></div></div>';
				}
	...
	static buildePersonalPanel() {
		...
		try {
			...
			for (var locIndex = 0; locIndex < jsonObj['addresses'].length; locIndex++) {
				addressesHtml +=
					'<div><p><b>' +
					jsonObj['addresses'][locIndex]['university'] +
					'</b> ' +
					jsonObj['addresses'][locIndex]['location'] +
					' (' +
					jsonObj['addresses'][locIndex]['hours'] +
					')</p></div>';
				addressesMobileHtml +=
					'<p><b>' +
					jsonObj['addresses'][locIndex]['university'] +
			...
			infoItemBtn.addEventListener('click', function (event) {
				if (document.getElementById('mobile-place-data').style.display == 'block') {
					document.getElementById('mobile-icon').innerHTML =
						'<svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 3.5C11.34 3.5 3.5 11.34 3.5 21C3.5 30.66 11.34 38.5 21 38.5C30.66 38.5 38.5 30.66 38.5 21C38.5 11.34 30.66 3.5 21 3.5ZM22.75 29.75H19.25V19.25H22.75V29.75ZM22.75 15.75H19.25V12.25H22.75V15.75Z" fill="#000000"></path></svg>';
					document.getElementById('mobile-place-data').style.display = 'none';
				} else {
					document.getElementById('mobile-icon').innerHTML =
						'<svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 3.5C11.34 3.5 3.5 11.34 3.5 21C3.5 30.66 11.34 38.5 21 38.5C30.66 38.5 38.5 30.66 38.5 21C38.5 11.34 30.66 3.5 21 3.5ZM22.75 29.75H19.25V19.25H22.75V29.75ZM22.75 15.75H19.25V12.25H22.75V15.75Z" fill="#5A67D8"></path></svg>';
	...
	static buildePageContent() {
		try {
			...
			for (
				var intrestIndex = 0;
				intrestIndex < jsonObj['researchInterests'].length;
				intrestIndex++
			) {
				researchInterestsHtml +=
					'<div class="intrest-item"><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 10H0V12H6V10ZM8.17 6.76L6.05 4.64L4.64 6.05L6.76 8.17L8.17 6.76ZM12 0H10V6H12V0ZM17.36 6.05L15.95 4.64L13.83 6.76L15.24 8.17L17.36 6.05ZM16 10V12H22V10H16ZM11 8C9.34 8 8 9.34 8 11C8 12.66 9.34 14 11 14C12.66 14 14 12.66 14 11C14 9.34 12.66 8 11 8ZM13.83 15.24L15.95 17.36L17.36 15.95L15.24 13.83L13.83 15.24ZM4.64 15.95L6.05 17.36L8.17 15.24L6.76 13.83L4.64 15.95ZM10 22H12V16H10V22Z" fill="#323232"/></svg>' +
					jsonObj['researchInterests'][intrestIndex] +
					'</div>';
			}
