...
class ProjectSection extends Element {
	...
	toHtml() {
		...
		// 2.
		var answer =
			'<div class="project-panel"><h3>' + this.name + '</h3>' + descriptionTrim(this.description);
		if (this.btn['link'] != '') {
			answer += '<div class="personal-row space-up-20 mobile-paddig-left">';

			if (this.btn['link'] != '' && this.btn['link'] != undefined) {
				answer +=
					'<div class="space-around"><a href="' +
					this.btn['link'] +
					'" class="download-btn"> Explore project </a></div>';
			}
			...
			if (this.btn['example'] != '' && this.btn['example'] != undefined) {
				answer +=
					'<div class="space-around"><a href="' +
					this.btn['example'] +
					'" class="secondary-btn"> See example </a></div>';
			}
