...
class Resource extends Element {
	...
	toHtml() {
		...
		if (this.recommendation != '') {
			answer += descriptionTrim(
				'<span class="recommend-promo"> Why am I recommending this? </span>' + this.recommendation,
			);
		}
		...
		for (var i = 0; i < this.fileLinks.length; i++) {
			if (this.fileLinks[i]['link'] != '' && this.fileLinks[i]['type'] + '' == '1') {
				answer +=
					'<a href="' +
					this.fileLinks[i]['link'] +
					'" class="download-btn acadmic-card-margin-fix">' +
					this.fileLinks[i]['info'] +
					'</a>';
			}
		}

		answer +=
			'<div class="w-100 acadmic-parms-row"><span>' +
			this.year +
			'</span><span>' +
			this.type +
			'</span></div>' +
