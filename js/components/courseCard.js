...
class CourseCard extends Element {
	...
	toHtml() {
		var answer =
			...
			'<p id="card-semeter" class="semester">Semster ' +
			this.semester +
			'  </p>' +
			...
			this.code +
			'" class="secondary-btn">Go to course</a></div>' +
			'</div>' +
			'</div>';
		return answer;
	}
	...
	static filterList(objList, property, filterValue) {
		...
		for (var objIndex = 0; objIndex < objList.length; objIndex++) {
			if (
				objList[objIndex][property + ''].trim().toLowerCase() == filterValue.trim().toLowerCase()
			) {
	...
	static splitByProperty(ObjList, property) {
		try {
			...
			for (var publicationIndex = 1; publicationIndex < ObjList.length; publicationIndex++) {
				if (
					ObjList[publicationIndex][property + ''].trim().toLowerCase() !=
					spliter.trim().toLowerCase()
				) {
