	...
	toHtml() {
		...
		if (this.fileLinks[0]['link'] != '' && this.isFileFormat(this.fileLinks[0]['link'])) {
			...
			answer +=
				...
				'" class="download-btn acadmic-card-margin-fix acadmic-download-btn">Download</a>';
			answer +=
				'<a href="' +
				this.fileLinks[0]['link'] +
				'" class="read-online-btn acadmic-card-margin-fix">Read Online</a></div>'; // added the Read online button and put both bottums inside the condition
			answer += '</div>';
		}
